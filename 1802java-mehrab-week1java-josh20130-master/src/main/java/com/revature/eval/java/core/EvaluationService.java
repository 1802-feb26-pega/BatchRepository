...
public class EvaluationService {
	...
	public String cleanPhoneNumber(String string) {
		String returnString = string.replace("^[0-9]", "");
		if (returnString.startsWith("1"))
		{
			return returnString.split("1", 1)[1];
		}
		return returnString;
	...
	static class BinarySearch<T extends Comparable<T>>{
		...
		public int indexOf(T t) {
			BinarySearch<T> searchList;
			int lengthOfList = sortedList.size();
			if (sortedList.size() == 1)//If target is instantly found
			{
				return 0;
			}
			if (sortedList.size() == 0)//If list is empty
			{
				return Integer.MIN_VALUE;
			}
			if(t.equals(sortedList.get(lengthOfList / 2)))//Center of list
			{
				return lengthOfList / 2;
			}
			if (t.compareTo(sortedList.get(lengthOfList/2)) < 0)
					{
				searchList = new BinarySearch<T>(sortedList.subList(0, lengthOfList  / 2 - 1)); //Checking bottom half of list
					return searchList.indexOf(t);
					}
				searchList = new BinarySearch<T>(sortedList.subList(lengthOfList / 2, + 1));//Checking Upper half of list
					return searchList.indexOf(t);
