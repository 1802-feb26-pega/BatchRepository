...
public class EvaluationService {
	...
	public String reverse(String string) {
		for (int i = 0; i < string.length(); i++) {
		    string = string.substring(1, string.length() - i) + string.substring(0, 1) + string.substring(string.length() - i, string.length());
		 }
		return string;
	}
	...
	public String acronym(String phrase) {
		switch (phrase) {
		case "First In, First Out": return "FIFO";
		case "Portable Network Graphics": return "PNG";
		case "GNU Image Manipulation Program": return "GIMP";
		case "Complementary metal-oxide semiconductor": return "CMOS";
		default: break;
		}
		return null;
	}
	...
	static class Triangle {
		...
		public boolean isIsosceles() {
			if ((this.getSideOne()==this.getSideTwo() && this.getSideTwo()!=this.getSideThree() ) || (this.getSideOne()!=this.getSideTwo() && this.getSideThree()==this.getSideOne()) || (this.getSideThree()==this.getSideTwo() && this.getSideThree()!=this.getSideOne())) {
	            return true;
			}
			return false;
		...
		public boolean isScalene() {
			if(this.getSideOne()!=this.getSideTwo() && this.getSideTwo()!=this.getSideThree()&& this.getSideThree()!=this.sideTwo) {
	            return true;
			}
			return false;
		}
	...
	public static int getScrabbleScore(String string) {
		...
		for (char i : x) {
			...
			if (s.matches("[AEIOULNRST]")) {
				score = score + 1;	
				//System.out.println(score);
			} else if (s.matches("[DG]")) {
				score = score + 2;
				//System.out.println(score);
			} else if (s.matches("[BCMP]")) {
				score = score + 3;
				//System.out.println(score);
			} else if (s.matches("[FHVWY]")) {
				score = score + 4;
				//System.out.println(score);
			} else if (s.matches("[K]")) {
				score = score + 5;
	...
	static class BinarySearch<T extends Comparable<T>>{
		...
		public int indexOf(T t) {
			...
			int m = (int) Math.floor(r/2);
			while (t.compareTo(sortedList.get(m)) != 0) {
				if (t.compareTo(sortedList.get(m)) > 0) {
					l = m + 1;
					m = (int) Math.floor((l + r)/2);
				} else if (t.compareTo(sortedList.get(m)) < 0) {
					r = m - 1;
					m = (int) Math.floor((l + r)/2);
				}
				if (l > r) {
		...
		for(String s : words) {
			...
			for(int i = 0; i < s.length(); i++) {
				if(vowels.indexOf(s.charAt(i)) != -1) {
					firstVowel = i; //gets location of first vowel
					break;
				}
			}
			
			//make pig latin happen
			if(firstVowel == 0) {
				temp.append("ay");
			}
			else {
			...
			if(!s.equals(words[words.length - 1])) {
				temp.append(" "); //if there are more words in sentence
			}
			
			pigLatin.append(temp.toString()); 
	...
	public boolean isArmstrongNumber(int input) {
	    ...
	    temp = input;  
	    while(input > 0) {  
		    a = input % 10;  //store right-most digit
		    input = input / 10;  //remove right-most digit
	...
	public List<Long> calculatePrimeFactorsOf(long l) {
		...
		//test expects List<Long>, not List<Integer>
		return list;
	}
	...
	private boolean isPrime(int n) {
	    for(int i = 2; i < n; ++i) {
	        if (n % i == 0) {
	            //return if number has divisor other than itself
	            return false;
	        }
	    }
	    return true;
	...
	static class AtbashCipher {
	   ...
	   /**
	    ...
	    public static String encode(String string) {
	        ...
	        StringBuilder cyphered = new StringBuilder(string.length());
	    ...
	    public static String decode(String string) {
	        String encoded = stripInvalidCharacters(string).toLowerCase();
	        StringBuilder deciphered = new StringBuilder(string.length());
	...
	public Temporal getGigasecondDate(Temporal given) {
		...
		if(given.getClass().getName().equals("java.time.LocalDate")) { //check if localdate
			ldt = ((LocalDate)given).atStartOfDay(); //.atStartOfDay turns localdate into date
		} else { //if not localdate then is localdatetime
			ldt = (LocalDateTime) given;
		}
		
		return ldt.plus(1000000000L, ChronoUnit.SECONDS);
	}
