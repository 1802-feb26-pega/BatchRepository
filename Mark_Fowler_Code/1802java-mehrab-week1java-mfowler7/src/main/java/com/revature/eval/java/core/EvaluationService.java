...
public class EvaluationService {
	...
	public String acronym(String phrase) {
		...
		String acro = "";
		String phraseReplace = phrase.replaceAll("-", " ");
		String[] words = phraseReplace.split(" ");
		
		for (String word : words) {
			acro += word.substring(0, 1).toUpperCase();
		}
		return acro;
	}
	...
	public Map<String, Integer> wordCount(String string) {
		...
		String parsed = string.replaceAll(",", " ").replaceAll("\n", "");
		String[] words = parsed.split(" ");
		// Track word-count pairs
	...
	public String toPigLatin(String string) {
		...
		// Split potential phrase argument into separate words
		String[] words = string.split(" ");
		// Will return string arg converted into Pig Latin
		String finalPhrase = "";
		// For each word collects the ending that will be added on
		String suffix = "";
		
		// Special cases
		// If word starts with a value
		for (int i = 0; i < words.length; i++) {
			if (words[i].substring(0, 1).matches("[aeiou]")) {
				finalPhrase += words[i] + "ay";
				continue;
			}
	    // If word starts with 'qu'
			if (words[i].substring(0, 2).matches("qu")) {
				finalPhrase += words[i].substring(2, words[i].length()) + "quay" + " ";
				continue;
			}
			
			// Loop over each word until a vowel is found
			boolean foundVowel = false;
			int counter = 0;
			
			while (!foundVowel) {
				if (words[i].substring(counter, counter + 1).matches("[aeiou]")) {
					finalPhrase += words[i].substring(counter) + suffix + "ay" + " ";
					break;
				}
				else {
					suffix += words[i].substring(counter, counter + 1);
					counter++;
				}
				
			}
			// Reset suffix variable at the end of each for loop
			suffix = "";
		}
	...
	public boolean isArmstrongNumber(int input) {
		...
		for (int i = 0; i < nums.length; i++) {
			sum += (Math.pow(nums[i], numString.length()));
		}
	...
	public boolean isValidIsbn(String string) {
		// Parse input for dashes
		String[] parsedArr = string.replaceAll("-", "").split("");
		int[] nums = new int[parsedArr.length];
		
		// Check to see if last character is numeric or an 'X'
		if (parsedArr[parsedArr.length - 1].equalsIgnoreCase("X")) {
			parsedArr[parsedArr.length - 1] = "10";
		}
		else if (parsedArr[parsedArr.length - 1].matches("[^0-9]")) {
			return false;
		}
		
		// Move values from String array into int array
		// Return false if non-numeric value is found
		try {
			for (int i = 0; i < parsedArr.length; i++) {
				nums[i] = Integer.parseInt(parsedArr[i]);
			}
		} catch (Exception ex) {
			return false;
		}
		
		// Add each number mulitpled by decrementing value (10 to 1)
		int total = 0;
		int counter = 10;
		for (int i = 0; i < 10; i++) {
	...
	public boolean isPangram(String string) {
		...
		// Create map to track number of times each letter is used
		Map<Character, Integer> letterTracker = new HashMap<>();
		Set<String> SetOfLetters = new HashSet<String>();
		for (char i = 'a'; i <= 'z'; i++) {
			letterTracker.put(i, 0);
		}
		
		// Parse string
		String[] letters = string.replaceAll(" ", "").split("");
	...
	public boolean isLuhnValid(String string) {
		...
		String[] parsedArr = string.replaceAll("[^0-9]", "").split("");
		int[] intArr = new int[parsedArr.length];
		
		for (int i = 0; i < parsedArr.length; i++) {
			intArr[i] = Integer.parseInt(parsedArr[i]);
		}
	...
	public int solveWordProblem(String string) {
		...
		switch (operator) {
		case "plus":
			answer = Integer.parseInt(words[2]) + Integer.parseInt(words[4]);
			break;
		case "minus":
			answer = Integer.parseInt(words[2]) - Integer.parseInt(words[4]);
			break;
		case "multiplied":
			answer = Integer.parseInt(words[2]) * Integer.parseInt(words[5]);
			break;
		case "divided":
			try {
				answer = Integer.parseInt(words[2]) / Integer.parseInt(words[5]);
